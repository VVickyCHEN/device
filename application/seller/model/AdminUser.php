...
class AdminUser extends Model
{
    ...
    // 设置当前模型对应的完整数据表名称
    protected $table = '__ADMIN_SELLER__';
    protected $pk = 'userid';
    ...
    public function createMerchant($data)
    {
        ...
        $data['password'] = $passwordinfo['password'];
        $data['encrypt'] = $passwordinfo['encrypt'];
        $id = Db::name('admin_seller')->insertGetId($data);
        if ($id) {
            return $id;
        }
        $this->error = '入库失败！';
    ...
    public function editMerchant($data)
    {
        ...
        $status = Db::name('admin_seller')->where('userid',$data['userid'])->update($data);
        return $status !== false ? true : false;
    }
    ...
    public function loginStatus($userId)
    {
        ...
        $city = $info;
        $da['aid']               = $userId;
        $da['sid']               = Session('seller_user_auth')['sid'];
        $da['rid']               = Session('seller_user_auth')['roleid'];
        $da['operation_type']    = '1';
        $da['operation_time']    = time();
        $da['operation']         = '登录操作';
        $admin = Db::name('admin_seller_operation')->insert($da);
    ...
    {
        $user = session('seller_user_auth');
        if (empty($user)) {
            return 0;
        } else {
            return session('seller_user_auth_sign') == data_auth_sign($user) ? $user['uid'] : 0;
        }
    }
    ...
    public function logout($sess=array())
    {
        if(empty($sess))
        {
            session('seller_user_auth',null);# adminauth
            session('seller_user_auth_sign',null);# adminauth
        }
        return true;
    }
