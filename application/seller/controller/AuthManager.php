...
use app\seller\model\AuthGroup;
use app\seller\model\AuthRule;
 ...
 * 权限管理控制器
 */
class AuthManager extends Sellerbase
{
    /**
     * 权限管理首页
     */
    ...
    {
        $list = Db::name('SellerAuthGroup')->where([['module' ,'=', 'seller'],['id','>',1]])->order(['id' => 'ASC'])->select();
    ...
    public function access()
    {
        ...
        $result = model('Menu')->returnNodes(false);
        $group_id = $this->request->param('group_id/d');
        $rules = Db::name('SellerAuthGroup')
            ->where('status', '<>', 0)
            ->where('id', '=', $group_id)
            ->where(['type' => AuthGroup::TYPE_ADMIN])
            ->value('rules');

        $map = array('module' => 'seller', 'status' => 1);
        $main_rules = Db::name('SellerAuthRule')->where($map)->column('name,id');
        $json = array();
    ...
    public function editGroup()
    {
        ...
        $auth_group = Db::name('SellerAuthGroup')->where(array('module' => 'admin', 'type' => AuthGroup::TYPE_ADMIN))->find($id);
