...
layui.define(['jquery', 'element', 'nprogress', 'utils'], function (exports) {
	...
	var Tab        = function () {
		this.config = {
			elem       : undefined,
			mainUrl    : '/admin/index/index.html',
			renderType : 'iframe',
			openWait   : true
		};
		this.v      = '1.0.5';
	...
	var _tab     = {
		...
		createTabDom   : function () {
			...
			var tpl  = [
					...
					'<li class="layui-this" lay-id="-1" data-url="' + _config.mainUrl + '"><i class="iconfont">&#58934;</i> 首页</li>',
					'</ul>',
		...
		tabAdd         : function (options) {
			$(".layui-tab-title .layui-tab-bar").remove();
			var that      = this,
				_config   = that._config,
				loadIndex = undefined;
			options       = options || {
			...
			titleHtm.push('</li>');
			var contentHtm = '<div class="layui-tab-item layui-show custom-tab-item" lay-item-id="' + id + '" >{{content}}</div>';
			switch (_config.renderType) {
				case renderType.page:
