;(function ($, document, window) {
    $.pictureViewer = function (options) {
        var pictureViewer_html =    '<div id="pictureViewer">' +
                                    '<div class="content">' +
                                    '<div class="menu-bar" style="height:40px;">' +
        ...
        var lockBody = function lockBody() {
            $('.picture-content img').css('transform','rotate(0deg)');
        ...
        $prevBtn.on('click', function(){
            toPrevImage();
            current = 0;
            $('.picture-content img').css({'transform':'rotate('+current+'deg)','transition':'no'});
            $('#pictureViewer').show();
            return false;
        });
        //handel next click
        ...
        //点击遮罩关闭
        $('#pictureViewer').on('click', hideView);
        //点击旋转      
        var current = 0;
        $('.totata_tu').click(function(){
            current++;
            $('.picture-content img').css({'transform':'rotate('+(90*current)+'deg)','transition':'0.5s'});
            $('#pictureViewer').show();
            return false;
        })
